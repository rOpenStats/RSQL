...
sql_execute_insert <- function(id_procesamiento, sql_insert,
  ...
  if ("db" %in% export){
    ...
    if (length(ret) > 0){
      ...
      #    if (nchar(ret[1])>5){
      #      print(paste("Para procesamiento", id_procesamiento, "en Insert No",
      #     contadorInserts, " se inserta error en la base"))
      #      print(paste(sql_insert))
      #      print(paste("Error:",res[1]))
      #      insertarErrorEnBD(id_procesamiento, contadorInserts, id_ciudad,
  ...
  if (!is.null(where_fields) &! is.null(where_values)){
    ...
    for (col in names(where_values)){
      if (max(is.character(where_values[, col])) == 1){
        ...
        new.values <- paste("'", sub("\\'([a-zA-Z0-9[:punct:]!'[:space:]]+)\\'", "\\1", where_values[, col]),
                            "'", sep = "")
...
sql_gen_insert <- function(table, insert_fields, values = c()){
  ...
  if (length(insert_fields) != ncol(values)){
    stop(paste("incompatible fields and data:", length(insert_fields), "!=", ncol(values),
               paste(insert_fields, collapse = ";"), paste(values, collapse = ";")))
  }
  separator <- ""
  sql_insert_fields <- ""
